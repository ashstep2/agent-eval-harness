...
Agents fail silently unless developers know exactly what to check for and when. Evals are hard.
...
**API primitive:** An evaluation helper in the SDK, with configurable dimensions, weight presets that encode product philosophy, cross-provider scoring by default.
...
**Month 1:** Map what agent builders are hacking around. Where do they build custom infra that should be in the SDK? (Interviews + X feedback + API usage pattern analysis.)
